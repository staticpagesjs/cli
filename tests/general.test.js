...
test('16 controller import myCustomController() when exists', async () => {
	await execute(['-c', 'config/16-controller-custom.yaml']);

	expect(getExpectedOutput('16-controller-custom')).toStrictEqual(getProducedOutput());
});
